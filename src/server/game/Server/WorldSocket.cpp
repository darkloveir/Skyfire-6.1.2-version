...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
    ...
    if (m_Session)
        TC_LOG_ERROR("network.opcode", "S->C: %s %s", m_Session->GetPlayerInfo().c_str(), GetOpcodeNameForLogging(pkt->GetOpcode(), true).c_str());
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    if (m_Session)
        TC_LOG_ERROR("network.opcode", "C->S: %s %s", m_Session->GetPlayerInfo().c_str(), opcodeName.c_str());
