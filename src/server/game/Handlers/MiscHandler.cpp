...
void WorldSession::HandleTimeSyncResp(WorldPacket& recvData)
{
    ...
    recvData >> SequenceIndex >> ClientTime;
    ...
    if (SequenceIndex != _player->m_timeSyncQueue.front())
        TC_LOG_ERROR("network", "Wrong time sync counter from player %s (cheater?)", _player->GetName().c_str());

    TC_LOG_DEBUG("network", "Time sync received: counter %u, client ticks %u, time since last sync %u", SequenceIndex, ClientTime, ClientTime - _player->m_timeSyncClient);

    uint32 ourTicks = ClientTime + (getMSTime() - _player->m_timeSyncServer);

    // diff should be small
    TC_LOG_DEBUG("network", "Our ticks: %u, diff %u, latency %u", ourTicks, ourTicks - ClientTime, GetLatency());

    _player->m_timeSyncClient = ClientTime;
...
void WorldSession::HandleDBQueryBulk(WorldPacket& recvPacket)
{
    ...
    uint32 count = recvPacket.ReadBits(13);

    DB2StorageBase const* store = GetDB2Storage(TableHash);
    if (!store)
    {
        TC_LOG_ERROR("network", "CMSG_DB_QUERY_BULK: %s requested unsupported unknown hotfix type: %u", GetPlayerInfo().c_str(), TableHash);
        return;
    }
    ...
    for (uint32 i = 0; i < count; i++)
    {
        ...
        ByteBuffer Data;
        store->WriteRecord(RecordID, (uint32)GetSessionDbcLocale(), Data);

        WorldPacket data(SMSG_DB_REPLY, 4 + 4 + 4 + Data.size());
